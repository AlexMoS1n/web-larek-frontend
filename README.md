...
Методы, геттеры и сеттеры:
 ...
 - `setCustomerInfo(userData: ICustomer): void`  - запись всей информации о покупке
 - `getCustomerInfo(): ICustomer` - возвращение всей информации о покупке.
...
#### Класс BasketHeader
Реализует корзину, расположенную в header страницы: показывает количество добавленного товара и открывает при нажатии полноценное модальное окно с подробной корзиной товаров.   

Поля:
- `protected basketButton: HTMLButtonElement` - кнопка корзины, расположенная в элементе header
...
Параметры в конструкторе:
- `container: HTMLElement` - DOM элемент, внутри которого располагается корзина 
...
#### Класс Form
Реализует пользовательский функционал с формами.  

Поля:
- `protected _form: HTMLFormElement` - форма, переданная в конструкторе
...
Параметры в конструкторе:
- `formConteiner: HTMLFormElement` - контейнер формы
- `events: IEvents` - объект класса `EventEmitter` для инициации событий при изменении данных.
...
- `get form(): HTMLFormElement` - возвращает форму
- `clear():void` - очистка формы.

#### Класс Card  
Отвечает за отображение карточки с товаром, задавая в карточке данные товара: название, изображение, категорию, описание и цену. Класс используется для отображения карточек на странице сайта. В конструктор класса передается DOM элемент темплейта, что позволяет при необходимости формировать карточки разных вариантов верстки. 

Поля:
- `protected _card: HTMLElement` - контейнер карточки, передаваемый в конструкторе
...
Поля:
- `protected _catalog: HTMLElement` - каталог карточек товаров 
- `protected classCard: ICardConstructor` - класс для создания карточки товара
- `protected events: IEvents` - объект, являющимся брокером событий, поддерживает интерфейс IEvents.

Параметры в конструкторе:
- `container: HTMLElement` - DOM элемент, контейнер для вывода карточек
- `classCard: ICardConstructor` - класс для создания карточки товара
- `events: IEvents` - объект, являющимся брокером событий, поддерживает интерфейс 
IEvents.
...
Методы, геттеры и сеттеры:
- `delCard(): void` - метод удаления карточки
- `set card(cardData: TCardBasket): void` - заполняет атрибуты элементов карточки данными
- `get card(): HTMLElement` - метод возвращает полностью заполненную карточку.

#### Класс BasketView
Отображает корзину с добавленными в нее товарами.
...
#### Класс Success
Уведомление об успешной покупке, содержит кнопку "за новыми покупками!".
